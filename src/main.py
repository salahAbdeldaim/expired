from database import get_db_path, init_db, get_connection 
import flet as ft
...
def contact_page(page: ft.Page):
    ...
    home_image_path = os.path.join(ASSETS_DIR, "home11.gif")
    image_content = (
        ft.Image(
            src=home_image_path,
            border_radius=16,
...
def export_page(page: ft.Page, nav: ft.NavigationBar, file_picker: ft.FilePicker):
    ...
    def export_to_pdf(e):
        try:
            filtered_data = load_data()  # Get filtered data
            ...
            def on_file_picker_result(e: ft.FilePickerResultEvent):
                if e.path:
                    pdf_path = os.path.join(e.path, f"items_export_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf")
                    try:
                        ...
                        doc.build([table])
                        show_success_snackbar(page, f"✅ PDF exported to {pdf_path}")
                    except Exception as ex:
                        logging.error(f"Error exporting PDF: {ex}")
                        show_error_snackbar(page, f"Error exporting PDF: {ex}")
                else:
                    show_error_snackbar(page, "No save location selected")

            file_picker.on_result = on_file_picker_result
            file_picker.get_directory_path(dialog_title="Select PDF Save Location")
            page.update()
...
def settings_page(page: ft.Page):
    return ft.Container(
        content=ft.Text(
            ...
            size=26,
            weight=ft.FontWeight.BOLD,
            color=page.theme.color_scheme.primary if page.theme and page.theme.color_scheme else ft.Colors.BLUE,
        ),
        alignment=ft.alignment.center,
        expand=True,
        bgcolor=page.theme.color_scheme.background if page.theme and page.theme.color_scheme else ft.Colors.WHITE,
    )
